...
class TestingDataset(Dataset):
    ...
    def __init__(self, data_path, image_size=1024, mode='test', requires_name=True, point_num=1, return_ori_mask=True,
                 prompt_path=None):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.prompt_path is None:
            try:
                ...
                print(f"错误发生在掩码路径: {mask_path}")
                return None  # 返回 None 以跳过这个数据
...
class TrainingDataset(Dataset):
    def __init__(self, data_dir, image_size=1024, mode='train', requires_name=True, point_num=1, mask_num=5):
        """
