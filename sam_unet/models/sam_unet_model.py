...
class SAM_UNET(nn.Module):
    ...
    def __init__(self,
        ...
        self.adapters_in = nn.ModuleList([Reshaper(img_size // 4, channel_list[0], num_patchs, embed_dim),
                                          # [b, channel_list[0], S/4 (64), S/4 (64)] --> [b, num_patches (16), num_patches (16), dim]
                                          Reshaper(img_size // 8, channel_list[1], num_patchs, embed_dim),
                                          # [b, channel_list[1], S/8 (32), S/8 (32)] --> [b, num_patches (16), num_patches (16), dim]                                   # [b, 2048, S/32 (8), S/32 (8)] --> [b, num_patches (16), num_patches (16), dim]
                                          Reshaper(img_size // 16, channel_list[2], num_patchs, embed_dim),
                                          # [b, channel_list[2], S/16 (16), S/16 (16)] --> [b, num_patches (16), num_patches (16), dim]
                                          Reshaper(img_size // 32, channel_list[3], num_patchs,
                                                   embed_dim), ])  # [b, channel_list[3], S/32 (8), S/32 (8)] --> [b, num_patches (16), num_patches (16), dim]
    ...
    def infer(self, list_input: List[Dict[str, Any]]) -> List[Dict[str, torch.Tensor]]:
        ...
        for i in range(len(self.global_index)):
            ...
            if i in range(len(self.global_index) - 1):  # 0, 1, 2
                ...
                current_out_feature_map = self.adapters_bridge[3 - i - 1](
        ...
        outputs = []
        for image_record, curr_embedding, multi_scale_feature in zip(list_input, image_embeddings,
                                                                     self.multi_scale_feature):
