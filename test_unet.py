...
import argparse
import os
from utils import FocalDiceloss, generate_point, save_masks,save_imgs_prompts_masks
...
def parse_args():
    ...
    # parser.add_argument("--run_name", type=str, default="MedSAM_unet(all32)_adapter", help="run model name")
    parser.add_argument("--run_name", type=str, default="unet_adapter_no_unet", help="run model name")
    ...
    parser.add_argument("--checkpoint", type=str, default="./work_dir/models/MedSAM_unet_adapter_no_unet/epoch10_sam.pth",help="sam checkpoint")
    # parser.add_argument("--checkpoint", type=str, default="./work_dir/models/MedSAM_unet(32)_adapter/epoch11_sam.pth",help="sam checkpoint")
    ...
    parser.add_argument("--iter_point", type=int, default=3, help="iter num")
...
def prompt_and_decoder(args, batched_input, model, image_embeddings, multi_scale_feature):
    ...
    low_res_masks = model.mask_decoder(
...
def main(args):
    ...
    model = sam_unet_registry['res34_sam_unet'](need_ori_checkpoint=False, sam_unet_checkpoint=args.checkpoint)
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        with torch.no_grad():
            ...
            # feature_maps_in = model.resnet(input_images)  # a list of feature maps for encoder
            feature_maps_out = [None] * 3  # a list of feature maps for decoder
            input_images = model.image_encoder.forward_patch_embed(input_images)
            for i in range(len(model.global_index)):
                for j in range(2):
                    input_images = model.image_encoder.forward_block(input_images, i * 3 + j)
                # current_feature_map = model.adapters_in[i](feature_maps_in[i])
                # current_feature_map = current_feature_map.permute(0, 2, 3, 1)
                input_images = input_images#  + current_feature_map
                input_images = model.image_encoder.forward_block(input_images, model.global_index[i])
                if i in range(len(model.global_index) - 1):  # 0, 1, 2
                    permuted_input_images = input_images.permute(0, 3, 1, 2)
                    current_out_feature_map = model.adapters_bridge[3 - i - 1](
                        permuted_input_images)  # 3 - i - 1 means 2, 1, 0, because the model is a U-Net
                    feature_maps_out[3 - i - 1] = current_out_feature_map
            image_embeddings = model.image_encoder.forward_neck(input_images)

            model.multi_scale_feature = model.embedding_encoder(image_embeddings)
            model.multi_scale_feature += feature_maps_out[0]
            model.multi_scale_feature += feature_maps_out[1]
            model.multi_scale_feature += feature_maps_out[2]

            multi_scale_feature = model.multi_scale_feature
        ...
        if args.boxes_prompt:
            ...
            masks, low_res_masks = prompt_and_decoder(args, batched_input, model, image_embeddings,multi_scale_feature)
            points_show = None
        ...
        if args.save_pred:
            if args.boxes_prompt:
                ...
                #                         points_show)
        focal_loss, dice_loss = criterion(masks, ori_labels)
